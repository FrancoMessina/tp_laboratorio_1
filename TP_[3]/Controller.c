...
int controller_loadFromBinary(char* path , LinkedList* pArrayListEmployee)
{
	...
	if(path != NULL && pArrayListEmployee != NULL)
	{
		...
		{
			if(parser_EmployeeFromBinary(p,pArrayListEmployee))
			{
				printf("Carga de archivo de texto exitosa\n");
				todoOk = 1;
			}
...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee != NULL){
		...
		{
				printf("Error no se puede ingresar mas de 19 caracteres \n");
		}
		stringPrimerLetraMayuscula(nombre);
		horas = ingresarEntero("Ingresar horas trabajadas : ");
		while(horas < 0 )
		{
			horas = ingresarEntero("Error... Ingresar horas trabajadas : ");
		}
		sueldo = ingresarEntero("Ingresar sueldo : ");
		while(sueldo < 0 )
		{
			sueldo = ingresarEntero("Error. Ingresar sueldo : ");
		}
		...
		if(newEmpleado != NULL)
		{
			if(employee_setId(newEmpleado,(id))
					&& employee_setNombre(newEmpleado,nombre)
					&& employee_setHorasTrabajadas(newEmpleado,horas)
					&& employee_setSueldo(newEmpleado,sueldo))
			{
...
//Id es el que ingresa el usuario, Si encuentro devuelvo el indice
Employee* controller_searchEmployeeID(LinkedList* pArrayListEmployee,int id, int* index)
{
	Employee* auxEmployee = NULL; //Empleado a analizar en cada iteracion de for
	int auxId;//Id del empleado que averiguo en cada iteracion de for
	*index = -1;
	for(int i = 0 ; i < ll_len(pArrayListEmployee) ; i++ )
	{
